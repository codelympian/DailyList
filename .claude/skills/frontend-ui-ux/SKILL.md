...
The reference library has one architecture file, 12 taste skills (Layer A — _how to execute_), and 70 brand design systems (Layer B — _what it should look like_). Most non-trivial tasks load **one Layer A + one Layer B**. `README.md` carries the full routing flow, stacking rules, anti-patterns, and the mandatory browser-based Design QA phase; `_INDEX.md` catalogs all 83 files with mood-to-brand mappings — read it whenever routing is not obvious from the tables below.
